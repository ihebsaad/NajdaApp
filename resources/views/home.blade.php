...
@section('content')
    ...
    <div class="card-header bg-danger " >
      ...
      <?php  //\App\Http\Controllers\DossierImmobileController::mettreAjourTableDossImmobile();
                //\App\Http\Controllers\DossierImmobileController::envoi_khaled_mail();
            
           $dossiers = App\DossierImmobile::get();  ?> 
 <div class="row">
       <div class="col-md-12">
        ...
        <table class="table table-striped" id="mytablef" >
            ...
            @foreach($dossiers as $dossier)
                ...
                <tr>
                    <td style="width:20%;"><a href="{{action('\App\Http\Controllers\DossiersController@view', $dossier['id'])}}" >{{$dossier->reference_medic}}</a> <a style="color:#a0d468" href="{{action('\App\Http\Controllers\DossiersController@fiche', $dossier['id'])}}" >Fiche<i class="fa fa-file-txt"></a></td>
                    ...
                    <td style="width:20%"><?php echo '<small>'.$dossier->langue_client.'</small>';?></td>
...
// Apply the search
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('#mytablef thead tr:eq(1) th').each( function () {
                var title = $('#mytablef thead tr:eq(0) th').eq( $(this).index() ).text();
                $(this).html( '<input class="searchfield" type="text"   />' );
            } );

            var table = $('#mytablef').DataTable({
                orderCellsTop: true,
                order:[],
...
// Apply the search
            ...
            table.columns().every(function (index) {
                ...
                $('#mytablef thead tr:eq(1) th:eq(' + index + ') input').on('keyup change', function () {
                ...
                $('#mytablef thead tr:eq(1) th:eq(' + index + ') input').keyup(delay(function (e) {
                    console.log('Time elapsed!', this.value);
                    $(this).blur();
