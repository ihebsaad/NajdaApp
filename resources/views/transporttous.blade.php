...
@section('content')
    ...
    $ordres_taxi = \App\OMTaxi::where('affectea', 'externe')->where('CL_heuredateRDV', '>', Carbon::now()->toDateString())
      ->where('dernier',1)->orWhere('affectea','interne')->where('CL_heuredateRDV',  '>', Carbon::now()->toDateString())->where('dernier',1)->where('complete',1)->where('statut','Validé')->orWhere('affectea','mmentite')->where('CL_heuredateRDV',  '>', Carbon::now()->toDateString())->where('dernier',1)->where('complete',1)->where('statut','Validé')
        ->select('id','CL_heuredateRDV','affectea','emplacement','reference_medic','subscriber_name','subscriber_lastname','CL_heure_RDV','CL_contacttel','CL_lieuprest_pc','CL_lieudecharge_dec','type','lchauff','lvehicule','dateheuredep','dateheuredispprev')
    ...
    $ordres_ambul =   \App\OMAmbulance::where('affectea' , 'externe')->where('CL_heuredateRDV',  '>', Carbon::now()->toDateString())
        ->where('dernier',1)->orWhere('affectea','interne')->where('CL_heuredateRDV',  '>', Carbon::now()->toDateString())->where('dernier',1)->where('complete',1)->where('statut','Validé')->orWhere('affectea','mmentite')->where('CL_heuredateRDV',  '>', Carbon::now()->toDateString())->where('dernier',1)->where('complete',1)->where('statut','Validé')
            ->select('id','CL_heuredateRDV','affectea','emplacement','reference_medic','subscriber_name','subscriber_lastname','CL_heure_RDV','CL_contacttel','CL_lieuprest_pc','CL_lieudecharge_dec','type','lvehicule','lambulancier1','dateheuredep','dateheuredispprev')
        ->orderBy('CL_heuredateRDV')
         ->get();
    // OM Remorq
...
$dateorigine ='01/01/1970,01:00';
                                                ...
                                                <div  class="om " style=" float:left;margin-top:15px;margin-right:30px;background-color:<?php echo $color2; ?>; color:white;  ;border-radius: 20px;padding:5px 5px 5px 5px;min-height: 120px"  id="div-<?php echo $type;?>-<?php echo $o['id'];?>"  ondblclick="display(this);">
                                                    ...
                                                    <div class="row" style= "margin-bottom:5px">
                                                        <div   style="background-color:white;color:black;text-align: center;font-size: 20px"><i class="fas fa-calendar"></i> <?php echo $dateom; ?></div> </div>
