...
$rol=$request->get('rol');
	$vers=$request->get('vers');
        \Session::put('variableTest'.$rol, 1);
        $par=$request->get('par');
 ...
 public function acceptpausesup(Request $request)
    {
...
$idusers=$user->id;
            $nomuser=$user->name.' '.$user->lastname;
            $nomagent=  app('App\Http\Controllers\UsersController')->ChampById('name',$idusers).' '.app('App\Http\Controllers\UsersController')->ChampById('lastname',$idusers);
	 ...
	 $hist = new Historique([
            ...
            User::where('id', $idusers)->update(array('statut'=>'2'));
